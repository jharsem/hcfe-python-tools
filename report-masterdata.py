...
sql = ['appt_px_all','Gls Appt No','Cl Appt No','cx_all','Gl Cx No','Cl Cx No','CL Cx Per RX','Total Appt No','OCT No','CL New Fit','Cl Refit','Unique Cx Per Month']
...
#dt_from = datetime.datetime(2019,6,1)
#dt_to = datetime.datetime(2019,6,30)
...
def sendReport(f):
    ...
    Report Notes
        ...
        Unique Cx in Month  # Unique Patients (no exclusion) per month (based on invoices, including Medicare) 
    ...
    msg['Subject'] = "HCFE Monthly Base Number Report for {} to {}".format(dt_from.strftime("%Y-%m-%d"),dt_to.strftime("%Y-%m-%d"))
    msg['From'] = fromaddr
    msg['To'] = toaddrs
